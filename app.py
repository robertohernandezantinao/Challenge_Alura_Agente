...
from ingestion import load_documents, build_vectorstore
try:
...
def ingest_files(files):
    ...
    for f in files:
        shutil.copy(f.name, os.path.join(
            DOCS_FOLDER, os.path.basename(f.name)))
    ...
    return f"✅ Índice actualizado con {len(docs)} fragmento(s) provenientes de '{DOCS_FOLDER}'."
...
def ask_agent(question):
    ...
    sources_text = "\n".join(
        f"- {s}" for s in sorted(set(final_sources))) if final_sources else "Sin fuentes."
...
with gr.Blocks(theme=gr.themes.Default(spacing_size="sm", text_size="sm")) as demo:
    ...
    with gr.Tab("📚 Cargar documentos"):
        file_uploader = gr.File(
            label="Documentos internos", file_count="multiple")
        ingest_button = gr.Button("Procesar y crear índice", variant="primary")
        ingest_status = gr.Textbox(
            label="Estado de la ingestión", interactive=False)
        ingest_button.click(
            fn=ingest_files, inputs=file_uploader, outputs=ingest_status)
        gr.Markdown(
            "_El repositorio ya incluye documentos de ejemplo en `sample_docs/`. "
            "Puedes usarlos directamente ejecutando `python ingestion.py`, "
            "o subir los tuyos aquí._"
        )
    ...
    with gr.Tab("💬 Preguntar al agente"):
        ...
        trace_box = gr.Textbox(
            label="Trazabilidad del agente (pasos del grafo)", lines=6)
        answer_box = gr.Textbox(label="Respuesta", lines=8)
        sources_box = gr.Textbox(label="Fuentes utilizadas", lines=4)

        ask_button.click(fn=ask_agent, inputs=question_box, outputs=[
                         trace_box, answer_box, sources_box])
